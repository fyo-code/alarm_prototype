...
function renderChrome() {
  ...
  const tabs = [["l0","tab_l0"],["l1","tab_l1"],["l2","tab_l2"],
                ["check","tab_check"],["lt","tab_lt"],["mail","tab_mail"]];
  $("tabs").innerHTML = tabs.map(([id,k]) =>
    `<button data-tab="${id}" class="${S.tab===id?"on":""}">${esc(T(k))}</button>`).join("");
...
async function viewL0() {
  ...
  v.innerHTML = `
    ...
    <div class="filters" style="margin-bottom:12px">
      <span class="sub" style="color:var(--ink-3)">${esc(T("focus_lbl"))}</span>
      ...
      ${S.focusFamily ? `<span class="tag">${esc(T("focus_note"))}</span>` : ""}
    </div>

    <div class="kpis">
      <div class="kpi"><div class="k">${esc(T("k_units"))}</div><div class="v">${n0(tot.units)}</div>
        <div class="s">${tot.skus_with_stock} ${esc(T("skus"))} · ${tot.factories} ${S.lang==="ro"?"fabrici":"factories"}</div></div>
      <div class="kpi"><div class="k">${esc(T("k_value"))}</div><div class="v money">${kLei(tot.value)}</div>
        <div class="s">${S.meta.config.money.basis === "cost" ? (S.lang==="ro"?"cost de achiziție":"acquisition cost") : ""}</div></div>
      <div class="kpi click" id="kStuck"><div class="k">${esc(T("k_stuck"))}</div>
        <div class="v">${pct(byBasis(stuck) / byBasis(tot) * 100)}</div>
        <div class="s">${esc(T("k_stuck_s"))} · ${S.basis === "units"
          ? `${pct(stuck.value / tot.value * 100)} ${S.lang==="ro"?"din bani":"of the money"}`
          : `${pct(stuck.units / tot.units * 100)} ${S.lang==="ro"?"din bucăți":"of the units"}`}</div></div>
      <div class="kpi click" id="kGreen"><div class="k">${esc(T("k_green"))}</div>
        <div class="v" style="color:var(--green)">${pct(byBasis(green) / byBasis(tot) * 100)}</div>
        <div class="s">${esc(T("k_green_s"))} · ${S.basis === "units"
          ? `${pct(green.value / tot.value * 100)} ${S.lang==="ro"?"din bani":"of the money"}`
          : `${pct(green.units / tot.units * 100)} ${S.lang==="ro"?"din bucăți":"of the units"}`}</div></div>
      <div class="kpi click" id="kReorder"><div class="k">${esc(T("k_reorder"))}</div>
        <div class="v">${n0(d.reorder_total.qty)}</div>
        <div class="s">${kLei(d.reorder_total.value)} · ${d.reorder_total.skus} ${esc(T("skus"))}</div></div>
    </div>

    <div class="card">
      <div class="card-head"><div class="grow">
        <h2>${esc(T("donut_t"))}</h2>
        <div class="hint">${esc(T("donut_h"))}</div>
      </div></div>
      <div class="grid g-photo" style="align-items:center">
        <div style="max-width:330px;margin:0 auto"><div id="donut"></div></div>
        <div class="tbl-wrap"><table class="legend" id="legend"></table></div>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:14px">
      <div class="card-head"><div class="grow">
        <h2>${esc(T("trend_t"))}</h2>
        <div class="hint">${esc(T("trend_h"))}</div>
      </div></div>
      <div id="chTrend"></div>
    </div>
  ...
  // donut
  const donutSegs = segs.filter((s) => s.colour !== "black")
    .map((s) => ({ ...s, label: SEGL(s.colour) }));
  drawDonut($("donut"), donutSegs, {
    colours: COLOURS,
    get: byBasis,
    fmt: (v) => S.basis === "units" ? `${n0(v)} ${T("units")}` : lei(v),
    centerValue: S.basis === "units" ? n0(tot.units) : kLei(tot.value),
    centerLabel: S.basis === "units" ? T("units") : T("k_value"),
    onClick: (s) => go("l2", { filter: { ...fpFilter(), colour: s.colour } }),
  });

  // legend with sub-states
  const totU = tot.units || 1, totV = tot.value || 1;
  $("legend").innerHTML = segs.map((s) => {
    const subs = (s.substates || []).filter((x) => x.units > 0 || x.skus > 0)
      .sort((a, b) => b.units - a.units);
    const main = `<tr class="seg" data-colour="${s.colour}">
      <td style="width:16px"><span class="sw" style="background:${COLOURS[s.colour]}"></span></td>
      <td>${esc(SEGL(s.colour))}</td>
      <td class="num"><b>${n0(s.units)}</b> <span class="dim">${T("seg_units")}</span></td>
      <td class="num dim">${pct(s.units / totU * 100)}</td>
      <td class="num"><b>${kLei(s.value)}</b></td>
      <td class="num dim">${pct(s.value / totV * 100)}</td>
      <td class="num dim">${s.skus} ${T("seg_skus")}</td>
    </tr>`;
    const rows = subs.length > 1 ? subs.map((x) => `<tr class="sub" data-sub="${x.substate}">
      <td></td><td colspan="2">↳ ${esc(SUBL(x.substate))}</td>
      <td class="num">${n0(x.units)}</td><td class="num">${kLei(x.value)}</td>
      <td class="num"></td><td class="num">${x.skus} ${T("seg_skus")}</td>
    </tr>`).join("") : "";
    return main + rows;
  }).join("");
  $("legend").querySelectorAll("tr.seg").forEach((tr) =>
    tr.addEventListener("click", () => go("l2", { filter: { ...fpFilter(), colour: tr.dataset.colour } })));
  $("legend").querySelectorAll("tr.sub").forEach((tr) =>
    tr.addEventListener("click", () => go("l2", { filter: { ...fpFilter(), substate: tr.dataset.sub } })));

  // top factories by proposed reorder + how well they sell
  const re = d.top_reorder;
  drawHBar($("chReorder"), re.rows.map((r) => ({
  ...
  // trend: independent of the family focus (importer history is at code level,
  // and the focus filter would make the series incomparable month to month)
  get("/api/trend").then((tr) => {
    const host = $("chTrend");
    if (!host) return;
    if (!tr.available) { host.innerHTML = `<div class="empty">${esc(tr.reason || "—")}</div>`; return; }
    const useValue = S.basis === "value";
    drawLine(host, tr.rows.map((r) => ({
      label: r.month.slice(0, 7), value: useValue ? r.value : r.units,
    })), {
      fmt: (v) => useValue ? kLei(v) : n0(v),
      colour: useValue ? COLOURS.blue : COLOURS.grey,
      yLabel: useValue ? T("trend_value") : T("trend_units"),
      note: (S.lang === "ro"
        ? `De la <b>${tr.first.month.slice(0,7)}</b> la <b>${tr.last.month.slice(0,7)}</b>: bucăți <b>${sgn(tr.delta_units_pct)}%</b>, valoare <b>${sgn(tr.delta_value_pct)}%</b>, deci cost mediu pe bucată <b>${sgn(tr.delta_avg_cost_pct)}%</b>.<br>
           <b>Atenție la interpretare:</b> coșul de produse nu e același — ${n0(tr.cohort.first_codes)} coduri la început, ${n0(tr.cohort.last_codes)} la final, doar <b>${n0(tr.cohort.in_both)}</b> în ambele. Creșterea valorii vine din înlocuirea gamei, nu din aprecierea aceluiași stoc. Alt punct de start dă altă poveste (aug 2022 → aug 2026: bucăți ${sgn(tr.alt_baselines?.aug22_units ?? 0)}%, valoare ${sgn(tr.alt_baselines?.aug22_value ?? 0)}%).`
        : `From <b>${tr.first.month.slice(0,7)}</b> to <b>${tr.last.month.slice(0,7)}</b>: units <b>${sgn(tr.delta_units_pct)}%</b>, value <b>${sgn(tr.delta_value_pct)}%</b>, so average cost per unit <b>${sgn(tr.delta_avg_cost_pct)}%</b>.<br>
           <b>Read with care:</b> it is not the same basket — ${n0(tr.cohort.first_codes)} codes at the start, ${n0(tr.cohort.last_codes)} at the end, only <b>${n0(tr.cohort.in_both)}</b> in both. The value rise comes from range replacement, not from the same stock appreciating. A different baseline tells a different story (Aug 2022 → Aug 2026: units ${sgn(tr.alt_baselines?.aug22_units ?? 0)}%, value ${sgn(tr.alt_baselines?.aug22_value ?? 0)}%).`),
    });
  }).catch(() => {});
  ...
  $("kStuck").onclick = () => go("l2", { filter: { colour: "blue" } });
  $("kGreen").onclick = () => go("l2", { filter: { colour: "green" } });
  $("kReorder").onclick = () => go("l2", { filter: { reorder_only: true } });
}
...
async function viewL1() {
  if (!S.factory) {
    ...
    drawHBar($("pick"), d.top_stock.rows.map((r) => ({
      ...
      colour: COLOURS.blue, factory: r.factory,
      secondary: `${n1(r.dead_pct)}% ${S.lang==="ro"?"mort":"dead"}`,
  ...
  v.innerHTML = `
    ...
    <div class="kpis">
      <div class="kpi"><div class="k">${esc(T("k_reorder"))}</div><div class="v">${n0(h.reorder_qty)}</div>
  ...
  drawVBar($("chWidth"), d.widths.slice(0, 12).map((w) => ({
    ...
    secondary: `${w.skus} ${T("seg_skus")}`, colour: COLOURS.blue, width: w.width_cm,
  })), { fmt: fmtBasis });
...
async function viewL2() {
  ...
  v.innerHTML = `
    ...
    <div class="page-head">
      <div><h1>${esc(T("l2_title"))}</h1><div class="sub">${esc(T("l2_sub"))}</div></div>
      <div class="spacer"></div>
...
async function openSku(sku) {
  ...
  $("drawerBody").innerHTML = `
    <div class="verdict"><span class="stripe" style="background:${COLOURS[r.colour]}"></span>
...
async function viewCheck() {
  ...
  drawCompare($("cmp"),
    { label: T("chk_simple_c"), value: d.overall.simple_months || 0, colour: "#8A8F98" },
    { label: T("chk_engine_c"), value: d.overall.engine_months || 0, colour: "#2F72C4" },
